...
export class UploadController {
  ...
  createUploadUrl = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const bodyParsed = parseOrThrow(uploadInitSchema, req.body);

      const result = await this.service.createUploadUrl(user.id, paramsParsed.clientId, {
        type: bodyParsed.type,
        contentType: bodyParsed.contentType,
        sizeBytes: bodyParsed.sizeBytes ?? null
      });
  ...
  listUploads = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await this.service.listUploads(user.id, paramsParsed.clientId);
